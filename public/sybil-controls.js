(function(){
  function applyTheme(t){
    ...
    var b=document.getElementById('sc-theme'); if(b) b.innerHTML = t==='dark' ? SUN : MOON;
  }
  ...
  function build(){
    ...
    var wrap=document.createElement('div'); wrap.id='sybil-controls';
    var theme=document.documentElement.classList.contains('dark')?'dark':'light';
    var tb=document.createElement('button'); tb.id='sc-theme'; tb.className='sc-btn';
    tb.title='Toggle light / dark'; tb.setAttribute('aria-label','Toggle theme');
    tb.innerHTML = theme==='dark' ? SUN : MOON;
    tb.onclick=function(){ applyTheme(document.documentElement.classList.contains('dark')?'light':'dark'); };
    wrap.appendChild(tb);
    document.body.appendChild(wrap);
    window.SybilControls={wrap:wrap, MOON:MOON, SUN:SUN};
  }
